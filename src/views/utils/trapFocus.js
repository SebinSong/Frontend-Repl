const trapFocus = {
  data () {
    return {
      // focusedElement used when $el is not available (ex: tooltip)
      focusedElement: null,
      focusableElements: [
  ...
  methods: {
    ...
    trapFocus (e) {
      ...
      if (e.key === 'Tab') {
        ...
        const focusableChilds = Array.from(el.querySelectorAll(this.focusableEments))
        ...
        if (e.shiftKey) { // Shift + Tab
          if (document.activeElement === firstFocusChild) {
            // tav backwards, so go back to last element
            e.preventDefault()
