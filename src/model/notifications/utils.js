 ...
 * 2. If 'MAX_COUNT_READ' is defined and more than 'MAX_COUNT_READ' read notifications remain, then:
 *  2a. discard every read notification oldaer than the 'MAX_COUNT_READ' newest ones.
 ...
 *  5f. discard unread notifications, older than first.
 * 6. Return the remaining notifications.
 */
export function applyStorageRules (notifications) {
...
export function compareOnAge (notificationA, notificationB) {
...
export function compareOnPriority (notificationA, notificationB) {
  ...
  // If the given notifications are both read or both unread,
  // then prefer the newer ones.
  if (notificationA.read === notificationB.read) {
    ...
    // Note: use a condition array so that it is easy to add/remove condifitons later.
    const conditions = [
